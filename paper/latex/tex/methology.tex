...
Note that, there might be different number of corrspoding entities for a
word. We just consider the top-$k$ entities for each entities set. An analysis of the impact of K is
given in section of experiments.

For word pairs ($w_m$, $w_n$), when we compute semantic relatedness between their corrspoding entities, we
will get $k*k$ relateness results. Inspired by \cite{acl/IacobacciPN15}, we combine all those relatedness
...
words $w_1$ and $w_2$. One is conventional approach \cite{BudanitskyH06} which considers just the closest entities
amonge multiple entities vectors pairs. There are $W_{w_1}$ and $W_{w_2}$ represent two entities sets associated with two
...
However this relatedness measure approach misses the other entities also contribute to the computing of relatedness.
In fact, psychological studies suggest that humans, while comparing similarity between a pair of words, consider
different meanings of two words but not only the closest pairs\cite{}. This claim can be directly popularized when computing
...
is not conforming with the way of human thinking.

Therefore, we propose another startegy for computing semantic relatedness, called $weighted$, in which
...
We can leverage this number to estimate the dominance of each specific entity resource in knowledge graph.
...
Besides, those entities which are closeer would play a more important role in computing relatedness measurement.
We model this by biasing the relatedness computation towards closer entities through a power function with
